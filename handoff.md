...
| `iv100.js`   | **IV100 CP 資料庫**(繁中名 + L20/L25 滿 IV CP, 1118 筆) | 幾乎不變 |
| `tools/annotate-cp.js` | **自動補 CP 腳本**(把 events.js 名稱補上 (L20/L25)) | 不用改 |
| `README.md`  | 使用者向的說明 | 少 |
| `handoff.md` | 本文件 | 視需要 |

網頁有三個分頁:**📅 活動行事曆**(讀 events.js)、**✈️ 全球花點座標**(讀 hotspots.js)、**💯 IV100查詢器**(讀 iv100.js)。

> `iv100.js` 也是「活動標註 CP」的查表來源(見第 4 節)。資料以官方種族值用標準 CP 公式算出,已與 pokemon.gameinfo.io 對驗一致。`l20`=等級20 滿 IV CP、`l25`=等級25 滿 IV CP。
...
2. **編輯 `events.js`**:照既有格式新增/修改 `{ }` 物件(見第 4 節),寶可夢名稱**先照原文寫、不用手打 CP**。
   - 若某活動已存在(例如社群日先前是 placeholder),**更新那一筆,不要新增重複**。
   - 先前用過「活動待公布(日期)」當佔位卡;官方公布後**直接改寫該卡**。
3. **自動補 CP**:`node tools/annotate-cp.js`(在 repo 根目錄)。會自動把名稱補上青綠色顯示的 `(L20/L25)`。看它印出的「對不到」清單,真名才需加 alias 再重跑(見第 4 節 ★)。
4. **驗證語法**(務必做,避免 JS 壞掉整頁空白):見第 6 節。
5. **部署**:見第 5 節。
6. **線上驗證 + 回報**:抓線上 events.js 確認關鍵字存在、活動筆數合理,再回報使用者網址。

> 一行版指令流(改完 events.js 後):
> ```bash
> node tools/annotate-cp.js && node -e "const fs=require('fs');Function(fs.readFileSync('events.js','utf8')+'\nreturn EVENTS;')();console.log('OK')" && git add -A && git -c user.name="Alexchiang-MF" -c user.email="miaofang0814@gmail.com" commit -m "更新活動" && git push
> ```
...
### ★ 重要規則:活動內容要標註 IV100 CP(已自動化)

使用者要求:寶可夢名稱後面要加上 `(L20/L25)` 兩個滿 IV CP 值(查 `iv100.js`)。
**不用手動查表 —— 直接跑腳本即可:**

```bash
node tools/annotate-cp.js     # 在 repo 根目錄執行
```

它會把 `events.js` 裡 highlights/note 的寶可夢名稱自動補上 `(L20/L25)`,保留註解與格式,**可重複執行不會重標**。執行後會列出「疑似名稱但對不到」的詞供你判斷。

- 格式:`名稱(L20/L25)`,異色 `*` 在括號**後面**:`急凍鳥(1743/2179)*`。
- 顯示:網頁會把 `(L20/L25)` 以**青綠色 `#5eead4`**(`.cp` class)標示,有別於白色內文。
- 腳本已內建別名與前綴處理:`騎拉帝納/龍捲雲/雷電雲/土地雲/眷戀雲`→化身/別種、`百戰勇者`→英雄、`暗影/極巨化`→本體、`卡璞・`符號、`起源形態`→起源、`大牙貍`→大牙狸。
- **遇到新的對不到名稱**(腳本會印出來):若確實是寶可夢,在 `tools/annotate-cp.js` 的 `alias` 物件加一條對應(對到 `iv100.js` 的繁中名)再重跑;若是敘述文字(如「幸運蛋」「付費自選限時調查」)則忽略。
- 夾在長敘述句中、非「、」分隔也非「」引號的名稱(少數),腳本可能漏標,必要時手動補。
