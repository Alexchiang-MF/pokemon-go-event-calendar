...
### ★ 重要規則:活動內容要標註 IV100 CP

使用者要求:**每次新增/更新活動時,在出現的寶可夢名稱後面加上 `(L20/L25)` 兩個滿 IV CP 值**(查 `iv100.js`)。

- 格式:`名稱(L20值/L25值)`。第一個 = 等級20 滿 IV CP,第二個 = 等級25。
  - 例:`噴火龍(1651/2064)`、`蓋歐卡(2351/2939)`。
- 若名稱原本有異色星號 `*`,放在括號**後面**:`急凍鳥(1743/2179)*`。
- 查表方式(從 `iv100.js` 找繁中名 `n`):
  ```bash
  grep -o '{"dex":[0-9]*,"n":"急凍鳥"[^}]*}' iv100.js   # 或用 node 載入 IV100 後 .find
  ```
- **要標哪些**:團體戰 Boss、社群日/聚焦時刻主角、團戰日對象、限時調查可遇的目標等「**會被捕捉/判斷 100%**」的寶可夢。一般敘述性提及可不標。
- **找不到對應**(如某些限定造型、敘述用語、地區未實裝):略過不標即可,別硬湊。
- 型態名稱要對應正確的 CP:例如「原始蓋歐卡」「闇黑酋雷姆」「超級噴火龍Y」在 `iv100.js` 都是獨立列,值不同,別只用基礎型。
